<div class="card border-theme-color shadow-sm position-relative">
	<a href="{{ route('news.read', ['id' => $info->id, 'slug' => \Str::slug(strtolower($info->title))]) }}" class="card-img d-block position-relative px-4" style="top: -50px; height: 220px;">
		<img src="{{ empty($info->image->url) ? '/images/news.jpg' : $info->image->url }}" class="img-fluid object-cover w-100 h-100 bg-theme-color p-4 rounded shadow-sm border-theme-color">
	</a>
